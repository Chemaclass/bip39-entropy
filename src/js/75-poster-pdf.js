...
function pdfDecoder(d, idx, x2, bottom){
  ...
  const label = t("pdf.decoderLabel");
  const math = t("pdf.decoderMath", { col: col * L.TR, row: row, idx: idx });
...
function pdfMasthead(d, s, colStart, rowStart){
  ...
  if (L.headH >= 15){
    ...
    const text = t("pdf.subtitle", { c: L.colBits, r: L.rowBits });
  ...
  if (L.sheets > 1){
    ...
    d.text(L.W - L.M, L.M + L.headH,
           t("pdf.sheetTag", { n: s + 1, of: L.sheets, lo: lo, hi: hi }),
...
function pdfSheet(d, s){
  ...
  d.text(L.M, fb, t("pdf.colophonLead"),
  ...
  d.text(L.W - L.M, fb, t("pdf.colophonSource"),
